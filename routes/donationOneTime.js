const express = require('express');
const Stripe = require('stripe');
const router = express.Router();
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY);

// Helper: pick an active one-time price for a product, prefer matching currency
async function getOneTimePriceForProduct(productId, desiredCurrency) {
  const list = await stripe.prices.list({
    product: productId,
    active: true,
    type: 'one_time',
    limit: 10,
  });
  if (!list.data.length) {
    const err = new Error('No active one-time Price for product');
    err.status = 400;
    throw err;
  }
  // try to match currency first
  const match = list.data.find(p => p.currency === (desiredCurrency || '').toLowerCase());
  return match || list.data[0];
}

router.post('/donation-one-time-payment-intent', express.json(), async (req, res) => {
  try {
    const { currency = 'usd', amount, product, hsPortalId, isCustom, metadata = {} } = req.body || {};
    if (!product || typeof product !== 'string') {
      return res.status(400).json({ error: 'product (Stripe Product ID) is required' });
    }
    
    // Resolve amount/currency depending on custom vs preset
    let resolvedAmount;
    let resolvedCurrency;
    let resolvedPriceId = null;

    const isCustomBool = (isCustom === true) || String(isCustom).toLowerCase() === 'true';

    if (isCustomBool) {
        // CUSTOM: use raw amount from client, and optionally create a Price for bookkeeping
        const n = Number(amount);
        if (!Number.isInteger(n) || n <= 0) {
            return res.status(400).json({ error: 'Valid amount (minor units) is required for custom donations' });
        }
        resolvedAmount = n;
        resolvedCurrency = String(currency || 'usd').toLowerCase();

        // OPTIONAL bookkeeping: create a (inactive) Price under this product
        try {
            const customPrice = await stripe.prices.create({
            product,
            unit_amount: resolvedAmount,
            currency: resolvedCurrency,
            active: false, // keep it off the public catalog
            nickname: `Custom ${resolvedAmount} ${resolvedCurrency}`,
            });
            resolvedPriceId = customPrice.id; // store for metadata
        } catch (e) {
            // Non-fatal: continue without a custom price if creation fails
            console.warn('[custom price create] non-fatal:', e?.message || e);
        }
    } else {
        // PRESET: derive amount from product's active one-time Price
        const price = await getOneTimePriceForProduct(product, currency);
        if (!Number.isFinite(price.unit_amount)) {
            return res.status(400).json({ error: 'Resolved price has no unit_amount' });
        }
        resolvedAmount = price.unit_amount;
        resolvedCurrency = price.currency;
        resolvedPriceId = price.id;
    }

    // Fetch the product to get its name
    const prod = await stripe.products.retrieve(product);

    const intent = await stripe.paymentIntents.create({
      amount: resolvedAmount,
      currency: resolvedCurrency, // use the price currency
      automatic_payment_methods: { enabled: true }, // lets the Element offer multiple PMs
      description: prod.name,
      metadata: {
        productId: product,
        hsPortalId: hsPortalId,
        isCustom: String(isCustomBool),
        ...(resolvedPriceId ? { priceId: resolvedPriceId } : {}),
        ...metadata, // your order/customer fields
      },
      // OPTIONAL: if you want a receipt, you can set receipt_email here instead of metadata
      // receipt_email: metadata.customer_email,
    });

    return res.json({ clientSecret: intent.client_secret });
  } catch (err) {
    console.error('[create-donation-payment-intent] ', err);
    return res.status(err.status || 500).json({ error: err.message || 'Failed to create PaymentIntent' });
  }
});

module.exports = router;